...
// Get all deluxe room images with room details
$rooms = $DELUXE_ROOMS;
...
<div class="w-full max-w-[1800px] mx-auto">
  <div class="mb-12 text-center">
    ...
    <p class="text-lg text-[#023e7d] max-w-2xl mx-auto">
      Experience luxury and comfort in our premium De Luxe rooms, designed for an unforgettable stay.
    </p>
  </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
    ...
    <article class="group rounded-3xl bg-white shadow-xl shadow-black/15 overflow-hidden transition-transform duration-300 hover:-translate-y-[3px] hover:shadow-2xl hover:shadow-black/20">
      <div class="relative aspect-[16/11] overflow-hidden rounded-tl-3xl rounded-tr-3xl">
        <img
          src="<?php echo $asset_base; ?>/assets/deluxe/<?php echo $room['image']; ?>"
      ...
      <div class="p-6">
        ...
        <ul class="mt-5 grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-slate-600">
          ...
          <li class="flex items-center gap-2"><span class="h-1.5 w-1.5 rounded-full bg-[#1e88e5]"></span>Premium bed</li>
        ...
        <a 
          href="<?php echo $basePath; ?>/?page=reservation&type=deluxe&room=<?php echo urlencode($room['name']); ?><?php echo $checkIn ? '&check_in=' . urlencode($checkIn) . '&check_out=' . urlencode($checkOut) : ''; ?>" 
